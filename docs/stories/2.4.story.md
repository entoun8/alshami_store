...
- [x] **Task 1: Audit Current Metadata State** (AC: 1, 2, 7)
  - [x] 1.1 Review all page.tsx files for existing metadata exports
  - [x] 1.2 Document which pages have titles vs missing titles
  - [x] 1.3 Document which pages have meta descriptions vs missing
  - [x] 1.4 Check root layout.tsx for base metadata configuration
  - [x] 1.5 Verify no duplicate metadata issues exist

- [x] **Task 2: Add/Update Static Page Metadata** (AC: 1, 2)
  - [x] 2.1 Update `app/(root)/products/page.tsx` - add description
  - [x] 2.2 Update `app/(root)/cart/page.tsx` - add description
  - [x] 2.3 Update `app/(root)/about/page.tsx` - add description
  - [x] 2.4 Update `app/(root)/contact/page.tsx` - add title and description
  - [x] 2.5 Update `app/(root)/user/profile/page.tsx` - add description
  - [x] 2.6 Update `app/(root)/user/orders/page.tsx` - add description
  - [x] 2.7 Update `app/(root)/shipping-address/page.tsx` - add title and description
  - [x] 2.8 Update `app/(root)/payment-method/page.tsx` - add title and description
  - [x] 2.9 Update `app/(root)/place-order/page.tsx` - add title and description
  - [x] 2.10 Update `app/(root)/order/[id]/page.tsx` - add description
  - [x] 2.11 Update `app/(auth)/sign-in/page.tsx` - add title and description
  - [x] 2.12 Update admin pages - add noindex for search exclusion

- [x] **Task 3: Add Dynamic Product Page Metadata** (AC: 1, 2, 3, 4)
  - [x] 3.1 Add `generateMetadata` function to `app/(root)/products/[slug]/page.tsx`
  - [x] 3.2 Fetch product data for metadata generation
  - [x] 3.3 Set dynamic title: "{Product Name} | Alshami"
  - [x] 3.4 Set dynamic description from product description (truncated if needed)
  - [x] 3.5 Add Open Graph metadata (og:title, og:description, og:image, og:type)
  - [x] 3.6 Add Twitter Card metadata (twitter:card, twitter:title, twitter:description, twitter:image)

- [x] **Task 4: Configure Open Graph for Key Pages** (AC: 4)
  - [x] 4.1 Update root layout metadata with default Open Graph config
  - [x] 4.2 Add og:site_name to root metadata
  - [x] 4.3 Add og:locale to root metadata
  - [x] 4.4 Add Twitter card defaults to root metadata
  - [x] 4.5 Ensure home/products page has proper OG image fallback

- [x] **Task 5: Add Canonical URLs** (AC: 5)
  - [x] 5.1 Verify `metadataBase` is set correctly in root layout
  - [x] 5.2 Update `SERVER_URL` in constants.ts for production URL
  - [x] 5.3 Add canonical URL configuration to product detail pages
  - [x] 5.4 Ensure canonical URLs use clean slugs (no query params)

- [x] **Task 6: Create robots.txt** (AC: 6)
  - [x] 6.1 Create `app/robots.ts` file using Next.js robots function
  - [x] 6.2 Allow crawling of public pages (/, /products, /products/\*, /about, /contact)
  - [x] 6.3 Disallow crawling of user pages (/user/_, /cart, /shipping-address, /payment-method, /place-order, /order/_)
  - [x] 6.4 Disallow crawling of admin pages (/admin/\*)
  - [x] 6.5 Disallow crawling of auth pages (/sign-in)
  - [x] 6.6 Reference sitemap in robots.txt

- [x] **Task 7: Create Sitemap** (AC: 6)
  - [x] 7.1 Create `app/sitemap.ts` file using Next.js sitemap function
  - [x] 7.2 Include static pages: /, /products, /about, /contact
  - [x] 7.3 Dynamically generate product page URLs from database
  - [x] 7.4 Set appropriate changeFrequency and priority values
  - [x] 7.5 Set lastModified dates (use product.created_at for products)

- [x] **Task 8: Admin Page Search Exclusion** (AC: 6)
  - [x] 8.1 Add `robots: { index: false, follow: false }` to admin layout or pages
  - [x] 8.2 Verify admin pages return noindex meta tag

- [x] **Task 9: Build and Verification** (AC: 1-7)
...
export const metadata: Metadata = {
  ...
  description:
    "Browse our curated selection of premium herbs, coffees, and more.",
};
```
...
1. **Metadata Template System**: Implemented title template in root layout (`%s | ${APP_NAME}`) so child pages only need to specify the page-specific title portion.

2. **Static Page Metadata**: Added `Metadata` exports with title and description to all 12 static pages (products, cart, about, contact, profile, orders, shipping-address, payment-method, place-order, order details, sign-in, admin layout).

3. **Dynamic Product Metadata**: Added `generateMetadata` function to product detail page with Open Graph and Twitter Card support. Description is truncated to 160 characters.

4. **Open Graph Configuration**: Root layout includes default OG metadata (type, siteName, locale) and Twitter card defaults. Product pages include product-specific OG metadata with images.

5. **Canonical URLs**: `metadataBase` was already set in root layout. Next.js automatically handles canonical URL generation based on this setting.

6. **robots.txt**: Created `app/robots.ts` with rules allowing public pages and disallowing user/admin/checkout pages. References sitemap.xml.

7. **Sitemap**: Created `app/sitemap.ts` that dynamically generates sitemap including static pages and all products from database with proper changeFrequency and priority values.

8. **Admin Noindex**: Added `robots: { index: false, follow: false }` to admin layout to prevent search engine indexing.

9. **Build Verification**: `npm run lint` and `npm run build` both pass. Build output confirms `/robots.txt` and `/sitemap.xml` are generated as static content.

### File List

| File | Action | Description |
|------|--------|-------------|
| `app/layout.tsx` | Modified | Added title template, Open Graph defaults, Twitter card defaults |
| `app/robots.ts` | Created | robots.txt generation with allow/disallow rules |
| `app/sitemap.ts` | Created | Dynamic sitemap generation including all products |
| `app/(root)/products/page.tsx` | Modified | Added metadata with title and description |
| `app/(root)/products/[slug]/page.tsx` | Modified | Added generateMetadata with OG and Twitter support |
| `app/(root)/cart/page.tsx` | Modified | Added metadata with description |
| `app/(root)/about/page.tsx` | Modified | Added metadata with description |
| `app/(root)/contact/page.tsx` | Modified | Added metadata with description |
| `app/(root)/user/profile/page.tsx` | Modified | Added metadata with description |
| `app/(root)/user/orders/page.tsx` | Modified | Added metadata with description |
| `app/(root)/shipping-address/page.tsx` | Modified | Added metadata with description |
| `app/(root)/payment-method/page.tsx` | Modified | Added metadata with description |
| `app/(root)/place-order/page.tsx` | Modified | Added metadata with description |
| `app/(root)/order/[id]/page.tsx` | Modified | Added metadata with description |
| `app/(auth)/sign-in/page.tsx` | Modified | Added metadata with title and description |
| `app/(admin)/layout.tsx` | Modified | Added robots noindex/nofollow for SEO exclusion |
