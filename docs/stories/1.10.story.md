...
- [x] **Task 1: Verify Pre-Deployment Readiness** (AC: 1)
  - [x] 1.1 Verify `.env.example` contains all required variables (already exists, verify completeness)
  - [x] 1.2 Run `npm run build` to ensure production build succeeds
  - [x] 1.3 Run `npm run lint` to ensure no linting errors
  - [x] 1.4 Verify all Stories 1.1-1.9 and Epic 2 are marked Done
  - [x] 1.5 Test production build locally with `npm start`

- [x] **Task 2: Create Vercel Project** (AC: 2)
  - [x] 2.1 Create new Vercel project via Vercel Dashboard
  - [x] 2.2 Connect GitHub repository to Vercel
  - [x] 2.3 Configure build settings:
    - Build Command: `npm run build`
    - Output Directory: `.next`
    - Install Command: `npm install`
    - Framework: Next.js
  - [x] 2.4 Verify initial deployment succeeds (without env vars, will fail at runtime - expected)
...
- [x] **Task 4: Configure Vercel Environment Variables** (AC: 4, 5)
  - [x] 4.1 Navigate to Vercel Project → Settings → Environment Variables
  - [x] 4.2 Add Supabase variables:
    - `NEXT_PUBLIC_SUPABASE_URL` (Client + Server)
    - `NEXT_PUBLIC_SUPABASE_ANON_KEY` (Client + Server)
    - `SUPABASE_SERVICE_ROLE_KEY` (Server only - CRITICAL: never expose to client)
  - [x] 4.3 Add NextAuth variables:
    - `NEXTAUTH_URL` = `https://alshami-store.vercel.app` (Server)
    - `NEXTAUTH_SECRET` = strong random 32+ char secret (Server)
  - [x] 4.4 Add Google OAuth variables:
    - `AUTH_GOOGLE_ID` (Server)
    - `AUTH_GOOGLE_SECRET` (Server)
  - [x] 4.5 Add Stripe variables (LIVE mode):
  ...
  - [x] 4.6 Add Resend variables:
    - `RESEND_API_KEY` (Server only)
    - `EMAIL_FROM_ADDRESS` = `orders@your-domain.com` (Server)
  - [x] 4.7 Add App variable:
    - `SERVER_URL` = `https://alshami-store.vercel.app` (Server)
  - [x] 4.8 Trigger redeployment after adding env vars

- [x] **Task 5: Configure Google OAuth for Production** (AC: 6)
  - [x] 5.1 Go to Google Cloud Console → APIs & Services → Credentials
  - [x] 5.2 Edit the OAuth 2.0 Client ID used for this project
  - [x] 5.3 Add production URI to "Authorized JavaScript origins": `https://alshami-store.vercel.app`
  - [x] 5.4 Add production URI to "Authorized redirect URIs": `https://alshami-store.vercel.app/api/auth/callback/google`
  - [x] 5.5 Save changes
  - [x] 5.6 Test Google Sign-In on production

- [x] **Task 6: Configure Stripe Webhook for Production** (AC: 7, 8)
  - [x] 6.1 Go to Stripe Dashboard → Developers → Webhooks
  - [x] 6.2 Click "Add endpoint"
  - [x] 6.3 Set endpoint URL: `https://alshami-store.vercel.app/api/webhooks/stripe`
  - [x] 6.4 Select events to listen to: `payment_intent.succeeded`
  - [x] 6.5 Create endpoint and copy the signing secret
  - [x] 6.6 Update `STRIPE_WEBHOOK_SECRET` in Vercel with production secret
  - [ ] 6.7 Switch Stripe to LIVE mode (Dashboard toggle)
  - [ ] 6.8 Verify live keys are set in Vercel (`pk_live_`, `sk_live_`)
  - [x] 6.9 Trigger redeployment
...
- Task 1.1: Added missing `SERVER_URL` to `.env.example`
- Task 1.2: Build succeeds with 19 routes
- Task 1.3: Lint passes
- Task 1.4: All stories 1.1-1.9 and 2.1-2.7 confirmed Done
- Task 1.5: Production server starts successfully
- Tasks 2-9: Require manual user action in external dashboards (Vercel, Google Cloud, Stripe, Resend). Note: Vercel MCP is available for programmatic project/env configuration.
- Task 2: Vercel project created via Dashboard import. GitHub repo `entoun8/alshami_store` connected. Initial deployment READY at `alshami-store.vercel.app`. Build auto-detected Next.js framework.
- Task 4: All 13 env vars set via Vercel REST API. `NEXTAUTH_URL` and `SERVER_URL` set to `https://alshami-store.vercel.app`. Redeployment triggered and confirmed READY. Site returns 200 OK.
- Task 5: Google OAuth configured by user for `alshami-store.vercel.app` domain.
- Task 6: Stripe webhook endpoint created in Sandbox mode, pointing to `alshami-store.vercel.app/api/webhooks/stripe`, listening for `payment_intent.succeeded`. New signing secret updated in Vercel. Note: Still using test keys (pk_test_, sk_test_) — switch to live mode deferred until ready for real payments.

### File List

| File | Action |
|------|--------|
| `.env.example` | Modified - Added `SERVER_URL` variable |
